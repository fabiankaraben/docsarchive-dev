...
The new `tsconfig.json` will then be used as the source of truth for TypeScript-specific build information like files and configuration. You can learn [about how TSConfigs works here](/typescript/5.1/project-configuration/what-is-a-tsconfig-json) and there is a [comprehensive reference here](/typescript/5.1/project-configuration/tsconfig-reference).
...
There is a series of mappings for common TypeScript settings, these are settings which map directly to [TypeScript cli options](/typescript/5.1/project-configuration/tsc-cli-options) and are used to help you write a more understandable project file. You can use the [TSConfig reference](/typescript/5.1/project-configuration/tsconfig-reference) to get more information on what values and defaults are for each mapping.
