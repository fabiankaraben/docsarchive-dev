...
|`[--allowSyntheticDefaultImports ↗](https://www.typescriptlang.org/tsconfig.html#allowSyntheticDefaultImports)`|`boolean`<br>|`true` if [`esModuleInterop`](/typescript/5.1/project-configuration/compiler-options#esModuleInterop) is enabled, [`module`](/typescript/5.1/project-configuration/compiler-options#module) is `system`, or [`moduleResolution`](/typescript/5.1/project-configuration/compiler-options#module-resolution) is `bundler`; `false` otherwise.<br>|
...
|`[--alwaysStrict ↗](https://www.typescriptlang.org/tsconfig.html#alwaysStrict)`|`boolean`<br>|`true` if [`strict`](/typescript/5.1/project-configuration/compiler-options#strict); `false` otherwise.<br>|
...
|Have recompiles in projects that use [`incremental`](/typescript/5.1/project-configuration/compiler-options#incremental) and `watch` mode assume that changes within a file will only affect files directly depending on it.<br>|
