...
Check out [“The select Tag”](/react/18.1/main-concepts/forms#the-select-tag) for detailed instructions.

### style {#style}

> Note
> Some examples in the documentation use `style` for convenience, but **using the `style` attribute as the primary means of styling elements is generally not recommended.** In most cases, [`className`](/react/18.1/api-reference/dom-elements#classname) should be used to reference classes defined in an external CSS stylesheet. `style` is most often used in React applications to add dynamically-computed styles at render time. See also [FAQ: Styling and CSS](/react/18.1/faq/faq-styling).
> 
...
If you set `suppressHydrationWarning` to `true`, React will not warn you about mismatches in the attributes and the content of that element. It only works one level deep, and is intended to be used as an escape hatch. Don’t overuse it. You can read more about hydration in the [`ReactDOM.hydrateRoot()` documentation](/react/18.1/api-reference/react-dom-client#hydrateroot).
