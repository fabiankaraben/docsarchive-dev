---
linkTitle: "Higher-Order Components"
...
React’s diffing algorithm (called [Reconciliation](/react/18.1/advanced-guides/reconciliation)) uses component identity to determine whether it should update the existing subtree or throw it away and mount a new one. If the component returned from `render` is identical (`===`) to the component from the previous render, React recursively updates the subtree by diffing it with the new one. If they’re not equal, the previous subtree is unmounted completely.
...
The solution for this problem is to use the `React.forwardRef` API (introduced with React 16.3). [Learn more about it in the forwarding refs section](/react/18.1/advanced-guides/forwarding-refs).
